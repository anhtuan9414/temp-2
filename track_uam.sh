...
for val in $threads; do 
    ...
    if [ -z "$lastblock" ]; then 
        ...
        sudo docker rm -f $val
        echo -e "${RED}Remove: $val - Not activated after 2 days${NC}"
        restarted_threads+=("$val - Not activated after 2 days")
        ((numberRestarted+=1))
